...
class CreditCardProcessingApplicationTests {
	...
	public void testGetAllCredits() {
		...
		Assertions.assertEquals(1,responseEntity.getBody().size(), "Response List is empty");
		CreditCardDetails creditCardDetails = responseEntity.getBody().get(0);

		Assertions.assertAll(() -> {
			assertEquals("Tim", creditCardDetails.getName(), "Name on Card does not match");
			assertEquals(73342639365L, creditCardDetails.getCardNumber(), "Card Number does not match");
			assertEquals(Double.valueOf(300000), creditCardDetails.getLimit(), "Card Limit does not match");
			assertEquals(Double.valueOf(100), creditCardDetails.getBalance(), "Card Balance doe not match");
		});
		Assertions.assertEquals("Tim",((CreditCardDetails)responseEntity.getBody().get(0)).getName(), "Response List does not match the class");
	}
